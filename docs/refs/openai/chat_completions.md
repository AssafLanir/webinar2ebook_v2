...
**Scope**: Chat Completions endpoints (Create, Get, List, Update, Delete) with Python examples
**Note**: If conflict with live docs, prefer live docs.
...
#### `seed` (`integer or null`) — **Optional** **Deprecated**
...
If set to `true`, the response is streamed to the client as it is generated using server-sent events (SSE). See the Streaming section for handling chunk events.
