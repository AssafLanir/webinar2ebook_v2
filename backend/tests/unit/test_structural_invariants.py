...
'''
    ...
    def test_detects_stitched_fragments(self):
        """Detect 'The effects The belief' stitched fragments."""
        from src.services.structural_invariants import find_token_corruption

        doc = '''## Chapter 1

The effects The belief that everything is possible highlights the opportunities ahead.

### Key Excerpts
'''
        violations = find_token_corruption(doc)
        assert len(violations) >= 1
        assert any(v["pattern"] == "stitched_fragments" for v in violations)

    def test_detects_missing_space_after_comma(self):
        """Detect 'this,pointing' missing space pattern."""
        from src.services.structural_invariants import find_token_corruption

        doc = '''## Chapter 1

It has only been since human technology that we managed to lift ourselves,pointing to progress.

### Key Excerpts
'''
        violations = find_token_corruption(doc)
        assert len(violations) >= 1
        assert any(v["pattern"] == "missing_space_after_comma" for v in violations)

    def test_detects_truncated_word(self):
        """Detect 'he ir,' truncated word pattern."""
        from src.services.structural_invariants import find_token_corruption

        doc = '''## Chapter 1

Environments usually kill their species, he ir, referring to extinction.

### Key Excerpts
'''
        violations = find_token_corruption(doc)
        assert len(violations) >= 1
        assert any(v["pattern"] == "truncated_word" for v in violations)

    def test_detects_orphaned_it(self):
        """Detect ', It challenges' orphaned pronoun pattern."""
        from src.services.structural_invariants import find_token_corruption

        doc = '''## Chapter 1

When it comes to human behavior, It challenges deterministic views.

### Key Excerpts
'''
        violations = find_token_corruption(doc)
        assert len(violations) >= 1
        assert any(v["pattern"] == "orphaned_it" for v in violations)

    def test_detects_orphan_participial(self):
        """Detect 'noting, For' orphan participial pattern."""
        from src.services.structural_invariants import find_token_corruption

        doc = '''## Chapter 1

David Deutsch captures this shift, noting, For generations, societies barely changed.
