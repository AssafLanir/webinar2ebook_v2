...
export function StyleControls({ config, onChange }: StyleControlsProps) {
  ...
  const isInterviewQA = config.book_format === 'interview_qa'
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      {isInterviewQA && (
        <div className="md:col-span-2 p-4 bg-slate-700/50 rounded-lg border border-slate-600">
          <div className="flex items-center gap-2 mb-2">
            <svg className="w-5 h-5 text-cyan-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span className="text-sm font-medium text-slate-200">Interview Q&A Mode Active</span>
          </div>
          <p className="text-xs text-slate-400 mb-2">
            The following settings are automatically configured to preserve the interview format:
          </p>
          <ul className="text-xs text-slate-400 space-y-1">
            <li className="flex items-center gap-2">
              ...
              Strict faithfulness (speaker's actual words)
            </li>
            <li className="flex items-center gap-2">
              <svg className="w-3.5 h-3.5 text-cyan-500" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
            ...
            <li className="flex items-center gap-2">
              ...
              No extrapolation (transcript only)
            </li>
          </ul>
        </div>
      )}
